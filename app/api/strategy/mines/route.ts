...
export async function GET(request: Request) {
  ...
  const sellSystemId = parsePositiveInt(searchParams.get("sellSystemId"))
  const originId = parsePositiveInt(searchParams.get("originId"))
  ...
  if (!sellSystemId) {
    return NextResponse.json(
      { error: translate(locale, "strategy.api.sellRequired") },
      { status: 400 }
    )
  }

  try {
    const snapshot = await getMineSitesForPlan({
      typeId,
      sellSystemId,
